...
public final class OAuthAuthenticationHandler extends AbstractPreAndPostProcessingAuthenticationHandler {
    ...
    protected boolean doAuthentication(Credentials credentials) throws AuthenticationException {
        ...
        logger.debug("provider(resource) : {}", provider);
        
        //logger.debug("****************** accessToken start ******************");
        // get access token
        try{
        	...
        	logger.error("accessToken error : {}", log);
        }
...
/*        try{
        	...
        	logger.debug("get Error... "+ex.getMessage());
        	ex.printStackTrace();
        }*/
        
        logger.info("hard coding here");
        credential.setUserId("a");
